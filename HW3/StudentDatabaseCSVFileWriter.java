    ...
    public static void writeData(StudentDatabase db) {
        ...
        for (int i=0; i<numStudents; i++) {
            ...
            */
            ...
            output.format("%d,%s,%s,\"%s,%s %s\",USCI, CSCI, %s, %d, %f, %f, %s, %f, %f",
            current.getStudentID(),current.getSex(),current.getEthnicGroup(),
            current.getLastName(), current.getFirstName(),current.getMiddleInitial(),
            current.getSubPlan(), current.getStrtLevel(), current.getTotal(), current.getGPA(),
            current.getFAFSA(), current.getTakePrgrs(), current.getFinancialNeed()
            );
